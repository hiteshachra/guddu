...
@section('content')
    ...
    <div class="card">
        <div class="card-header border-bottom d-flex justify-content-between">
            ...
            <a href="{{ route('add_document_type') }}" class="btn add-new btn-primary"><span><span
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            const table1 = initializeDataTable("#data-table", "{{ route('document_type_list') }}", Columns, function (d) {
                d.status = $('#status').val();
            });
        ...
        function deleteType(typeId) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    let url = "{{ route('status_document_type', ':id') }}".replace(':id', typeId);
