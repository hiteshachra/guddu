          ...
          <div class="col-md-12">
            <div class="card card-primary">
              <div class="card-header">
                <div class="row">
                  <div class="col-sm-12 col-md-6 mt-1"><h3 class="card-title">Fund Request List</h3></div>
              ...
              <div class="card-body">
                  ...
                  <tr>
                    ...
                    <td>{{$value->amount}}</td>
                    <td>
                         <img src="{{ asset('images/fund_requests/' . $value->utr_img) }}"
                           height="25"
                           style="cursor: pointer;"
                           onclick="showImageModal('{{ asset('images/fund_requests/' . $value->utr_img) }}')">
                    </td>
                    <!-- <td>{{$value->points}}</td> -->
                    <td>{{$value->desc}}</td>
                    <td>{{$value->remark}}</td>
                    ...
                    <td>
                      ...
                      @php
                        $urlv = url('fund/request-update/verify/'.$value->id);
                        $urlr = url('fund/request-update/reject/'.$value->id);
                      @endphp
                        <div class="btn-group">
                          <button type="button" class="btn btn-success bg-gradient btn-xs" onclick="openModal('{{ $urlv }}', '{{ $value->fromUser->name }}', 'verify')">Verify</button>
                          <button type="button" class="btn btn-danger bg-gradient btn-xs" onclick="openModal('{{ $urlr }}', '{{ $value->fromUser->name }}', 'reject')">Reject</button>
                        </div>
                        @endif
                    </td>
                  </tr>
    ...
    <form id="idForm" action="" method="GET">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Fund Request <span id="kyc-statuss"></span> </h5>
